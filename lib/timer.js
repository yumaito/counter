...
function elapsedTime(start,goal){
  ...
  var milli = elapsed.getUTCMilliseconds();
	//年追加
	target.append(returnSpan('val',affixZero2(year)));
	target.append(returnSpan('un','年'));
	//月追加
	target.append(returnSpan('val',affixZero2(month)));
	target.append(returnSpan('un','ヶ月'));
	//日を追加
	target.append(returnSpan('val',affixZero2(date)));
	target.append(returnSpan('un','日'));
	//時間を追加
	target.append(returnSpan('val',affixZero2(hours)));
	target.append(returnSpan('ul','時間'));
	//分
	target.append(returnSpan('val',affixZero2(minutes)));
	target.append(returnSpan('un','分'));
	//秒
	target.append(returnSpan('val',affixZero2(seconds)));
	target.append(returnSpan('un','秒'));
	//ミリ秒
	target.append(returnSpan('val',affixZero3(milli)));
  //
  return target;
}
/*
