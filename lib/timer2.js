...
function elapsedTime(start,goal,target){
  ...
  var elapsed = new Date(goal-start);
	...
	//日を追加
	$(target + ' #day').empty();
	$(target + ' #day').append(returnSpan('val date',('0000'+date).slice(-4)));
	//$(target + ' #day').append(returnSpan('un date','DAYS'));
	//時間を追加
	$(target + ' #time').empty();
	$(target + ' #time').append(returnSpan('val hours',('0'+hours).slice(-2)));
	$(target + ' #time').append(returnSpan('ul hours',':'));
	//分
	//$(target + ' #time').empty();
	$(target + ' #time').append(returnSpan('val minutes',('0'+minutes).slice(-2)));
	$(target + ' #time').append(returnSpan('un minutes',':'));
	//秒
	//$(target + ' #time').empty();
	$(target + ' #time').append(returnSpan('val seconds',('0'+seconds).slice(-2)));
	$(target + ' #time').append(returnSpan('un seconds',' '));
	//ミリ秒
	//$(target + ' #time').empty();
	$(target + ' #time').append(returnSpan('val milliseconds',('00'+milli).slice(-3)));
  //
  //return target;
}
/*
